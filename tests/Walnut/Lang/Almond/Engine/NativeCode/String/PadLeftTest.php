...
final class PadLeftTest extends CodeExecutionTestHelper {
	...
	public function testPadLeftReturnType(): void {
		$result = $this->executeCodeSnippet("pad[str: 'hello', len: 10];",
			valueDeclarations: "pad = ^[str: String<4..8>, len: Integer<10..12>] => String<10..12> :: 
				#str->padLeft[length: #len, padString: '-'];"
		);
		$this->assertEquals("'-----hello'", $result);
	}
	...
	public function testPadLeftInvalidParameterKeys(): void {
		$this->executeErrorCodeSnippet('Parameter type [length: Integer[10]] is not a subtype of [length: Integer, padString: String]', "'hello'->padLeft[length: 10];");
	}
