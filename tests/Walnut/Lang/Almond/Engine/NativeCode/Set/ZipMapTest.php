...
final class ZipMapTest extends CodeExecutionTestHelper {
	...
	public function testZipMapReturnTypeFinite(): void {
		$result = $this->executeCodeSnippet("zipMap[['a'; 'bcd'; 'ef'], [1, 3, 2]]",
			valueDeclarations: "
				zipMap = ^p: [Set<String<1..3>, 2..5>, Array<Integer<0..10>, 3..6>] => Map<String<1..3>:Integer<0..10>, 2..5> :: 
					p.0->zipMap(p.1);
			"
		);
		$this->assertEquals("[a: 1, bcd: 3, ef: 2]", $result);
	...
	public function testZipMapReturnTypeInfiniteTarget(): void {
		$result = $this->executeCodeSnippet("zipMap[['a'; 'bcd'; 'ef'], [1, 3, 2]]",
			valueDeclarations: "
				zipMap = ^p: [Set<String<1..3>, 2..>, Array<Integer<0..10>, 3..6>] => Map<String<1..3>:Integer<0..10>, 2..6> :: 
					p.0->zipMap(p.1);
			"
		);
		$this->assertEquals("[a: 1, bcd: 3, ef: 2]", $result);
	...
	public function testZipMapReturnTypeInfiniteParameter(): void {
		$result = $this->executeCodeSnippet("zipMap[['a'; 'bcd'; 'ef'], [1, 3, 2]]",
			valueDeclarations: "
				zipMap = ^p: [Set<String<1..3>, ..5>, Array<Integer<0..10>, 3..>] => Map<String<1..3>:Integer<0..10>, ..5> :: 
					p.0->zipMap(p.1);
			"
		);
		$this->assertEquals("[a: 1, bcd: 3, ef: 2]", $result);
