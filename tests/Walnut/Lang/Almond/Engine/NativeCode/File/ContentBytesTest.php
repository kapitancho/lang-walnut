...
use Walnut\Lang\Test\Almond\Engine\NativeCode\File\FileAccessHelper;

final class ContentBytesTest extends FileAccessHelper {

	public function testContentOk(): void {
		file_put_contents($this->tempFilePath, "hi!");
		$result = $this->executeCodeSnippet(
			"{File[path: '$this->tempFilePath']}->contentBytes;",
	...
	public function testContentInvalidParameterType(): void {
		$this->executeErrorCodeSnippet(
			...
			"{File[path: '$this->tempFilePath']}->contentBytes(42);",
			typeDeclarations: "File := \$[path: String]; CannotReadFile := \$[file: File];			"
		);
	}
