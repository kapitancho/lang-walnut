...
final class IntersectionTypeNormalizerTest extends AlmondBaseTestHelper {
    ...
    public function testEnumSubsetTypes(): void {
		$this->userlandTypeBuilder->addEnumeration($e = new TypeName('E'), [
			$a = new EnumerationValueName('A'),
			$b = new EnumerationValueName('B'),
			$c = new EnumerationValueName('C'),
			new EnumerationValueName('D'),
		]);
	    $this->assertEquals(
         "E[A]", (string)$this->intersection(
             $this->typeRegistry->union([
                 $this->valueRegistry->enumeration($e, $a)->type,
	             $this->valueRegistry->enumeration($e, $b)->type
             ]),
             $this->typeRegistry->union([
	             $this->valueRegistry->enumeration($e, $a)->type,
				 $this->valueRegistry->enumeration($e, $c)->type
             ]),
         )
     );
    }
    ...
    public function testAliasTypes(): void {
	    $this->userlandTypeBuilder->addAlias(new TypeName('M'), $this->typeRegistry->boolean);
        self::assertEquals("(Integer&False)", (string)$this->intersection(
            $this->typeRegistry->userland->alias(new TypeName('M')),
            $this->typeRegistry->integer(),
            $this->typeRegistry->false,
            $this->typeRegistry->real()
        ));
	...
	public function testAliasing(): void {
		$this->userlandTypeBuilder->addAlias(new TypeName('M'), $this->typeRegistry->boolean);
		$this->userlandTypeBuilder->addAlias(new TypeName('N'), $this->typeRegistry->integer());
		self::assertEquals("(M&N)", (string)$this->intersection(
			$this->typeRegistry->userland->alias(new TypeName('M')),
			$this->typeRegistry->userland->alias(new TypeName('N')),
		));
	}
