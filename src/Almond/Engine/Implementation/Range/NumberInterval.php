...
use BcMath\Number;
use Walnut\Lang\Almond\Engine\Blueprint\Range\InvalidNumberInterval;
...
final readonly class NumberInterval implements NumberIntervalInterface {
	...
	public function contains(Number $value): bool {
		if ($this->start instanceof NumberIntervalEndpointInterface) {
		...
		if ($this->end instanceof NumberIntervalEndpointInterface) {
	...
	private static function checkIntersectionHelper(NumberIntervalInterface $i1, NumberIntervalInterface $i2, bool $asIntegerInterval): bool {
		if (!$i1->end instanceof NumberIntervalEndpointInterface) {
			return true;
		}
		if (!$i2->start instanceof NumberIntervalEndpointInterface) {
			return true;
		}
		$e = $i1->end->value;
		$s = $i2->start->value;
