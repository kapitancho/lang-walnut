...
namespace Walnut\Lang\Almond\LSP\Implementation\Server;
...
use Walnut\Lang\Almond\LSP\Blueprint\Compilation\CompilationIndexFactory;
use Walnut\Lang\Almond\LSP\Blueprint\Document\CompilationCache;
...
use Walnut\Lang\Almond\LSP\Blueprint\Feature\CompletionProvider;
use Walnut\Lang\Almond\LSP\Blueprint\Feature\DefinitionProvider;
use Walnut\Lang\Almond\LSP\Blueprint\Feature\DiagnosticsProvider;
use Walnut\Lang\Almond\LSP\Blueprint\Feature\FoldingRangeProvider;
use Walnut\Lang\Almond\LSP\Blueprint\Feature\HoverProvider;
use Walnut\Lang\Almond\LSP\Blueprint\Feature\InlayHintProvider;
...
use Walnut\Lang\Almond\LSP\Blueprint\Server\LspServer;
use Walnut\Lang\Almond\LSP\Blueprint\Transport\LspTransport;
use Walnut\Lang\Almond\LSP\Implementation\Document\BasicCompilationSnapshot;
...
final class JsonRpcLspServer implements LspServer {
    ...
    private function recompileAndPublish(string $uri, int $version, string $content): void {
        $moduleName  = $this->documentStore->uriToModuleName($uri);
        $sourceKey   = $this->documentStore->uriToSourceKey($uri);
        ...
        // The source key is the path relative to the source root with the full
        // extension (e.g. "foo.test.nut"), which is what the precompilers use.
        $compiler = $this->compiler
            ->withAddedSourceFinder(new InMemorySourceFinder([$sourceKey => $content]))
    ...
    private function buildPeerSourceFinder(string $excludeModuleName): InMemorySourceFinder {
        ...
        foreach ($this->compilationCache->listUris() as $uri) {
            ...
            if ($snapshot !== null && $snapshot->moduleName !== $excludeModuleName) {
                // Key with the original source extension so precompilers can identify
                // the file type (e.g. "foo.test.nut" → TestPrecompiler, "foo.nut" → EmptyPrecompiler).
                $sources[$this->documentStore->uriToSourceKey($uri)] = $snapshot->sourceText;
            }
        }
